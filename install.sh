
sudo cp -u start.conf /etc/nginx/conf.d/start.conf
  sudo cp -u mediawiki.conf /etc/nginx/conf.d/mediawiki.conf
  sudo cp -u lernen.conf /etc/nginx/conf.d/lernen.conf
  sudo cp -u php.conf /etc/nginx/conf.d/php.conf
  sudo cp -u typo3.conf /etc/nginx/conf.d/typo3.conf
  sudo cp -u wordpress.conf /etc/nginx/conf.d/wordpress.conf
  sudo cp -u joomla.conf /etc/nginx/conf.d/joomla.conf
  sudo cp -u drupal.conf /etc/nginx/conf.d/drupal.conf

